...
const Solides = () => {
  ...
  const fetchSolides = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`http://192.168.0.125:8000/api/solides/baby/${babyId}/`);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing) {
        await axios.put(`http://192.168.0.125:8000/api/solides/${editing.id}/`, payload);
      } else {
        await axios.post('http://192.168.0.125:8000/api/solides/', payload);
      }
  ...
  const handleDelete = async (id: number) => {
    Alert.alert('Confirmation', 'Supprimer ce solide ?', [
      { text: 'Annuler', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            await axios.delete(`http://192.168.0.125:8000/api/solides/${id}/delete/`);
            fetchSolides();
          } catch (e) {
