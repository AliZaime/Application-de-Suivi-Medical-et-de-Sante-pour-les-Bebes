...
const TeteePage = () => {
  ...
  const fetchTetees = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`http://192.168.1.108:8000/api/tetees/baby/${babyId}/`);
      console.log(babyId);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing) {
        await axios.put(`http://192.168.1.108:8000/api/tetees/${editing.id}/`, payload);
      } else {
        await axios.post(`http://192.168.1.108:8000/api/tetees/`, payload);
      }
    ...
    Alert.alert('Confirmation', 'Supprimer cette tétée ?', [
      { text: 'Annuler', style: 'cancel' },
      {
        text: 'Supprimer', style: 'destructive', onPress: async () => {
          try {
            await axios.delete(`http://192.168.1.108:8000/api/tetees/${id}/delete/`);
            fetchTetees();
          } catch (e) {
