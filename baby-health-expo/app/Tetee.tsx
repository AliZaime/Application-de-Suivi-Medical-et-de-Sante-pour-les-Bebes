...
const Tetee = () => {
  ...
  const fetchTetees = async () => {
    try {
      const response = await axios.get(`http://192.168.11.111:8000/tetees/baby/${babyId}/`);
  ...
  fetchTetees();
    ...
    try {
        ...
        const response = await axios.put(`http://192.168.11.111:8000/tetees/${editingTetee.id}/`, tetee);
        setTeteeList((prev) =>
            prev.map((item) => (item.id === editingTetee.id ? response.data : item))
        );
        } else {
        // Ajouter une nouvelle tétée
        const response = await axios.post(`http://192.168.11.111:8000/tetees/`, tetee);
  ...
  const handleDeleteTetee = async (id) => {
    try {
      await axios.delete(`http://192.168.11.111:8000/tetees/${id}/delete/`);
