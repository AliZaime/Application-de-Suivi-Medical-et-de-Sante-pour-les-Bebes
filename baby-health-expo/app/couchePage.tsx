...
const CouchePage = () => {
  ...
  const fetchCouches = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`http://192.168.0.125:8000/api/couches/baby/${babyId}/`);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing) {
        await axios.post(`http://192.168.0.125:8000/api/couches/${editing.id}/`, payload);
      } else {
        await axios.post(`http://192.168.0.125:8000/api/couches/`, payload);
      }
    ...
    Alert.alert('Confirmation', 'Supprimer cette couche ?', [
      { text: 'Annuler', style: 'cancel' },
      {
        text: 'Supprimer', style: 'destructive', onPress: async () => {
          try {
            await axios.delete(`http://192.168.0.125:8000/api/couches/${id}/delete/`);
            fetchCouches();
          } catch (e) {
