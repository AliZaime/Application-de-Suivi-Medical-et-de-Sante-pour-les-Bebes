...
const Temperature = () => {
  ...
  const fetchTemperatures = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`http://192.168.57.8:8000/api/temperatures/baby/${babyId}/`);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing) {
        await axios.put(`http://192.168.57.8:8000/api/temperatures/${editing.id}/`, payload);
      } else {
        await axios.post(`http://192.168.57.8:8000/api/temperatures/`, payload);
      }
  ...
  const handleDelete = async (id) => {
    Alert.alert('Confirmation', 'Supprimer cette température ?', [
      { text: 'Annuler', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            await axios.delete(`http://192.168.57.8:8000/api/temperatures/${id}/delete/`);
            fetchTemperatures();
          } catch (e) {
