...
const Today = () => {
  ...
  useEffect(() => {
    const fetchAdvices = async () => {
      setIsLoading(true);
      try {
        const response = await axios.get('http://192.168.11.104:8000/api/advice/');
  ...
  useEffect(() => {
    const fetchChildrenSchedules = async () => {
      try {
        ...
        const response = await axios.get(`http://192.168.11.104:8000/api/user/get_children_schedules/${parentId}/`, {
          headers: {
