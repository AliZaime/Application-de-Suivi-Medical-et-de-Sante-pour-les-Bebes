...
const AppointmentPage = () => { // Renamed component
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await axios.get(`http://192.168.1.107:8000/api/user/get_appointments_by_parent_id/${parentId}`);
  ...
  const daletAppiontemment = async (appointmentId: number) => {
    try {
      ...
      await axios.delete(
        `http://192.168.1.107:8000/api/user/delete_appointment/${appointmentId}/`,
        {
          headers: {
  ...
  const addAppointment = async () => {
    ...
    try {
      ...
      await axios.post(
        'http://192.168.1.107:8000/api/user/add_appointment/',
        newAppointmentData,
        {
          headers: {
  ...
  const saveEditedAppointment = async () => {
    ...
    try {
      ...
      await axios.put(
        `http://192.168.1.107:8000/api/user/update_appointment/${editingAppointment?.appointment_id}/`,
