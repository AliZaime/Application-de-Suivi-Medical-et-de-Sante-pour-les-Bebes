...
const Sommeil = () => {
  ...
  const fetchSommeils = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`http://192.168.57.8:8000/api/sommeils/baby/${babyId}/`);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing) {
        await axios.put(`http://192.168.57.8:8000/api/sommeils/${editing.id}/`, payload);
      } else {
        await axios.post('http://192.168.57.8:8000/api/sommeils/', payload);
      }
    ...
    Alert.alert('Confirmation', 'Supprimer cet enregistrement de sommeil ?', [
      { text: 'Annuler', style: 'cancel' },
      {
        text: 'Supprimer', style: 'destructive', onPress: async () => {
          try {
            await axios.delete(`http://192.168.57.8:8000/api/sommeils/${id}/delete/`);
            fetchSommeils();
          } catch (e) {
