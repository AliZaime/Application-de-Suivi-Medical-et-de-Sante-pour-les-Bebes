...
const Sommeil = () => {
  ...
  const fetchSommeils = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${config.API_BASE_URL}/api/sommeils/baby/${babyId}/`);
  ...
  const showDateTimePicker = (initialDate, onConfirm) => {
    ...
    Alert.alert('Confirmation', 'Supprimer cet enregistrement de sommeil ?', [
      { text: 'Annuler', style: 'cancel' },
      {
        text: 'Supprimer', style: 'destructive', onPress: async () => {
          try {
            await axios.delete(`${config.API_BASE_URL}/api/sommeils/${id}/delete/`);
            fetchSommeils();
          } catch (e) {
      ...
      <View style={styles.modalBg}>
        <View style={styles.modalContent}>
          ...
          <TouchableOpacity
              ...
              onPress={() => {
                if (Platform.OS === 'android') {
                  showDateTimePicker(dateDebut, setDateDebut);
                } else {
            ...
            >
              <Text>
                {dateDebut
                  ? dateDebut.toLocaleString()
                  : "Sélectionnez date et heure de début"}
              </Text>
            </TouchableOpacity>
          ...
          <TouchableOpacity
              ...
              onPress={() => {
                if (Platform.OS === 'android') {
                  showDateTimePicker(dateFin, setDateFin);
                } else {
            ...
            >
              <Text>
                {dateFin
                  ? dateFin.toLocaleString()
                  : "Sélectionnez date et heure de fin"}
              </Text>
            </TouchableOpacity>
          ...
          {(duration > 0 ) ? (
              ...
              <Text style={[styles.durationText, {color : 'red'}]}>Durée : {duration} min</Text>
          )}
