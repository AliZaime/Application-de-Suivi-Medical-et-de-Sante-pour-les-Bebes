...
const Biberon = () => {
  ...
  const fetchBiberons = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`http://192.168.57.8:8000/api/biberons/baby/${babyId}/`);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing) {
        await axios.put(`http://192.168.57.8:8000/api/biberons/${editing.id}/`, payload);
      } else {
        await axios.post('http://192.168.57.8:8000/api/biberons/', payload);
      }
  ...
  const handleDelete = async (id: number) => {
    Alert.alert('Confirmation', 'Supprimer ce biberon ?', [
      { text: 'Annuler', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            await axios.delete(`http://192.168.57.8:8000/api/biberons/${id}/delete/`);
            fetchBiberons();
          } catch (e) {
